...
class Publisher(sqs.SQSTalk):
    ...
    def publish(self, task_class, task_id, payload,
        ...
        task_queue = q_info.get_queue(task_class.queue)

        if task_class.time_limit >= task_queue.visibility_timeout_sec:
            raise exceptions.InvalidTimeLimitTaskException(
                'Invalid task time limit: %d >= %d from %s' % (
                    task_class.time_limit, task_queue.visibility_timeout_sec,
    ...
    def publish_messages_to_dead_letter_queue(self, task_queue, messages):
        """Sends a batch of messages to the dead letter queue.

        :param kale.queue_info.TaskQueue task_queue: name to send these messages to.
        ...
        """
        sqs_dead_letter_queue = self._get_or_create_queue(task_queue.dlq_name)
