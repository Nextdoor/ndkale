...
The publish() function is a static method of a task class.  Other than the first parameter, which can usually be None, it has the same signiture as the run\_task() method. A worker process, which may run on a different machine, will pick up the message and execute run\_task() method of the task.
